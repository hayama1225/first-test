      ...
      <div class="form">
        ...
        <script>
          document.addEventListener('DOMContentLoaded', function() {
            const destroyBase = "{{ url('/admin/contacts') }}"; // /admin/contacts を基底URLに
            ...
            document.querySelectorAll('.detail-btn').forEach(function(btn) {
              ...
              <p><strong>氏名:</strong> ${esc(data.last_name)} ${esc(data.first_name)}</p>
              <p><strong>性別:</strong> ${esc(gMap[data.gender] ?? data.gender)}</p>
              <p><strong>メール:</strong> ${esc(data.email)}</p>
              <p><strong>電話:</strong> ${esc(data.tel)}</p>
              <p><strong>住所:</strong> ${esc(data.address)} ${esc(data.building ?? '')}</p>
              <p><strong>種類:</strong> ${esc(data.category?.content ?? '')}</p>
              <p><strong>内容:</strong> ${esc(data.detail)}</p>
              <p><strong>作成日:</strong> ${esc(data.created_at)}</p>

              <!-- ▼モーダル内の削除フォーム -->
              <form method="POST" action="${destroyBase}/${id}"
                    onsubmit="return confirm('ID ${esc(id)} を削除します。よろしいですか？');"
                    style="margin-top:12px;">
                <input type="hidden" name="_token" value="{{ csrf_token() }}">
                <input type="hidden" name="_method" value="DELETE">
                <button type="submit" class="form__button-submit" style="height:auto;padding:6px 10px;width:100%;">
                  モーダルから削除
                </button>
              </form>
            `;
